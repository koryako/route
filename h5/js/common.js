;
!function () {
	var url_host = 'http://180.76.148.87:3168/';//更改接口域名
    ...
    var ajaxService = {
		...
		urlObj : {
        listbypage : 'getbypage',//分页列表
        tabweek : 'plan/perfdst/tabweek',//周目标
        tabday : 'plan/perfdst/tabday',//日目标
        tabquarter : 'plan/perfdst/index',//季度目标
        Detail : 'plan/perfdst/detail',//详情
        update:'plan/perfdst/update',
		Login : 'login',//登录
        add:'process_get',
        list:'getlist',
        del:'del',
        group:'getgroup', //组织架构
        logout : 'manage/manager/index?dowhat=logout',//退出登录
        ...
        send: function (callback) {
			...
			ajaxObj.success = function (response) {
				...
				/*var prepage=hmd.getFile();//获取当前加载的html页面
				if (prepage=='index.html'){					
					hmd.setPrepage('index.html');//刷新前一个页面为index.html				
				}				
				if (prepage=='myInfo.html'){
					
					hmd.setPrepage('myInfo.html');//刷新前一个页面为 我的页面
									
				}				
				if (prepage=='biaodiDetail.html'){
					if (response.financing!=undefined&&response.financing.waresId!=undefined){
					hmd.setwareid(response.financing.waresId);//刷新wareid										
				    }
					hmd.setPrepage('biaodiDetail.html');//刷新前一个页面为 标的详情页面					
				}				
				if (prepage=='buy.html'){
					if (response.wareInfo!=undefined&&response.wareInfo.waresId!=undefined){
					hmd.setwareid(response.wareInfo.waresId);//刷新 购买页面的wareid											
				    }
					hmd.setPrepage('buy.html');//刷新 前一个页面为  购买页面					
				}
				
				var jiebang;
				if (response.NewbieStepbonus){					
					 jiebang=response.NewbieStepbonus.bindCardRedPackage.unbindCard;					
				}
									  
                 if  (jiebang==1){
                       //已解绑
			             console.log('已解绑'); 
			             response.jiebangsate='1';	
		                 hmd.setjiebang('1');//设置解绑状态，1为解绑  0位没有解绑
                 }else{		
                 		 response.jiebangsate='0';
			             hmd.setjiebang('0');
		         }
				 //设置绑卡流程状态	
			     hmd.getNewbieStepbonus(response,function(value){
				          response.h5FrontEndstatecode=value;		
                          hmd.setStateCode(value);			 
			     });
			     //设置新浪支付状态	
		         hmd.getSinaStatus(response,function(value){			   
			              response.h5xinlangzhifustatecode=value;	
                          hmd.setzhifuStateCode(value);			   
		         });*/		   
        ...
        PostService: function () {
            ...
            return {
                run: function (url, data, callback) {
					...
					ajaxObj.url=self.port+url;
    /*switch (ispreRender){
		case 'new' : //提示售出
			ajaxObj.url = self.port + '/index.php?__=' + url + '&__VIEW__=jsonp&__preRender__=NewbieStepbonus';
			break;
		case 'xinlang' :   //请求新浪           
			ajaxObj.url = self.port + '/index.php?__=' + url + '&__VIEW__=jsonp&__preRender__=GetSinaStatus';
			break;
		case 'all' :   //都请求状态
			ajaxObj.url = self.port + '/index.php?__=' + url + '&__VIEW__=jsonp&__preRender__=NewbieStepbonus,GetSinaStatus';
			break;
		case 'no' :   //不请求状态		
			ajaxObj.url = self.port + '/index.php?__=' + url + '&__VIEW__=jsonp';	
			break;
		  default :
			ajaxObj.url = self.port + '/index.php?__=' + url + '&__VIEW__=jsonp';	
			break;
    ...
    parseUrl :function(url) {
	  ...
	  setStore:function (name,value){
		...
		//}else{
			   ...
			   this.setCookie(name,a,'.xiaoxialicai.com',30* 24 * 60 * 60); 
